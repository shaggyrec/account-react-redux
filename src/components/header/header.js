        ...
        if (activeObject && activeObject.ACTIVE === 'Y') {
            return (
                <ul className="navbar-nav hidden-sm-down">
                    ...
                    <li className="nav-item white-text">
                        <h2 className="navbar__objectName" dangerouslySetInnerHTML={{__html: activeObject.NAME}}/>
                    </li>
                    <li className="nav-item white-text">
                ...
                <li>
                    <ul>
                        <li className="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children">
                            ...
                            <div id="item0"
                                 className={`collapsible-body collapse ${activeObject && (url === `/lk/${activeObject && activeObject.ID}` || url === `/lk/${activeObject && activeObject.ID}/main` || url === `/lk/${activeObject && activeObject.ID}/desc` || url === `/lk/${activeObject && activeObject.ID}/photo` || url === `/lk/${activeObject && activeObject.ID}/services` || url === `/lk/${activeObject && activeObject.ID}/actions` || url === `/lk/${activeObject && activeObject.ID}/children` || url === `/lk/${activeObject && activeObject.ID}/conditions` || url === `/lk/${activeObject && activeObject.ID}/contacts` || url === `/lk/${activeObject && activeObject.ID}/how-to-get-here`|| url === `/lk/${activeObject && activeObject.ID}/distances` || url.indexOf(`/lk/${activeObject && activeObject.ID}/rooms`) !== -1) ? 'show' : ''}`}>
                                <ul>
            ...
            return (
                <header>
                        ...
                        <li>
                            <ul className="collapsible collapsible-accordion">
                                ...
                                <li><Link className="collapsible-header waves-effect arrow-r" to="/lk/faq"><i
                                    ...
                                    Помощь</Link></li>
                                <li>
                                    <Link className="collapsible-header waves-effect arrow-r" to="/lk/feedback">
                                        <i className="fa fa-eye"/>
                                        Обратная связь
                                    </Link>
                                </li>
                                <li>
                                    <a className={`collapsible-header waves-effect arrow-r`} data-toggle="collapse" href="#settings">
                                        <i className="fa fa-cog"/>
                                        Настройки
                                        <i className="fa fa-angle-down rotate-icon"/>
                                    </a>
                                    <div id="settings"
                                         className={`collapsible-body collapse${url.indexOf('/lk/profile') === 0 || url.indexOf('/lk/security') === 0 || url.indexOf('/lk/subordinate') === 0 ? ' show' : ''}`}>
                                        <ul>
                                            <li>
                                                <Link to="/lk/profile" className="waves-effect">Профиль</Link>
                                            </li>
                                            <li>
                                                <Link to="/lk/subordinate" className="waves-effect">Контактные лица</Link>
                                            </li>
                                            <li>
                                                <Link to="/lk/security" className="waves-effect">Безопасность</Link>
                                            </li>
                                        </ul>
                                    </div>
                                </li>
                    ...
                    <nav className="navbar fixed-top navbar-toggleable-md navbar-dark red double-nav">
                        ...
                        <div className="d-flex flex-column">
                            <Link to="/lk/" className="navbar-brand"/>
                            {this.renderAdminLinks(authenticatedUser, activeObject)}
                        </div>
                        { this.renderLinks() }
                        {this.renderUserbar(authenticatedUser)}
